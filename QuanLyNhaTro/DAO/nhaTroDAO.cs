...
namespace QuanLyNhaTro.DAO
{
    public class nhaTroDAO
    {
        ...
        {
            string sql = "select distinct NT.maNhaTro, NT.maChuNha, NT.diaChi, NT.gia, NT.limit , NT.status, HDTT.soSinhVien from nhaTro NT left join ( select HD.soSinhVien ,NT.maNhaTro from hopDong HD, thanhToan TT , nhaTro NT where HD.soSinhVien > 0 and TT.maNhaTro = NT.maNhaTro and HD.maThanhToan = TT.maThanhToan and TT.status = 0 ) HDTT on NT.maNhaTro = HDTT.maNhaTro where HDTT.sosinhvien >= 0 or HDTT.sosinhvien is null";
        ...
        public List<nhaTroList> SearchNhaTro(string searchItem)
        {
            ...
            string cmdDiaChi = "select distinct NT.maNhaTro, CN.ten, NT.diaChi, NT.gia, NT.limit , NT.status, HDTT.soSinhVien, CN.sdt, CN.diaChiChu from chuNha CN, nhaTro NT left join ( select HD.soSinhVien ,NT.maNhaTro from hopDong HD, thanhToan TT , nhaTro NT where HD.soSinhVien > 0 and TT.maNhaTro = NT.maNhaTro and HD.maThanhToan = TT.maThanhToan and TT.status = 0 ) HDTT on NT.maNhaTro = HDTT.maNhaTro where HDTT.sosinhvien >= 0 and NT.maChuNha = CN.maChuNha and dbo.fuConvertToUnsign1 (nt.diachi) LIKE N'%' + dbo.fuConvertToUnsign1 (N'" + searchItem + "') + '%' or HDTT.sosinhvien is null and NT.maChuNha = CN.maChuNha and dbo.fuConvertToUnsign1 (nt.diachi) LIKE N'%' + dbo.fuConvertToUnsign1 (N'" + searchItem + "') + '%' ";
            string cmdChuNha = "select distinct NT.maNhaTro, CN.ten, NT.diaChi, NT.gia, NT.limit , NT.status, HDTT.soSinhVien, CN.sdt, CN.diaChiChu from chuNha CN, nhaTro NT left join ( select HD.soSinhVien ,NT.maNhaTro from hopDong HD, thanhToan TT , nhaTro NT where HD.soSinhVien > 0 and TT.maNhaTro = NT.maNhaTro and HD.maThanhToan = TT.maThanhToan and TT.status = 0 ) HDTT on NT.maNhaTro = HDTT.maNhaTro where HDTT.sosinhvien >= 0 and NT.maChuNha = CN.maChuNha and dbo.fuConvertToUnsign1 (CN.ten) LIKE N'%' + dbo.fuConvertToUnsign1 (N'" + searchItem + "') + '%' or HDTT.sosinhvien is null and NT.maChuNha = CN.maChuNha and dbo.fuConvertToUnsign1 (CN.ten) LIKE N'%' + dbo.fuConvertToUnsign1 (N'" + searchItem + "') + '%' ";
            string cmdSDT = "select distinct NT.maNhaTro, CN.ten, NT.diaChi, NT.gia, NT.limit , NT.status, HDTT.soSinhVien, CN.sdt, CN.diaChiChu from chuNha CN, nhaTro NT left join ( select HD.soSinhVien ,NT.maNhaTro from hopDong HD, thanhToan TT , nhaTro NT where HD.soSinhVien > 0 and TT.maNhaTro = NT.maNhaTro and HD.maThanhToan = TT.maThanhToan and TT.status = 0 ) HDTT on NT.maNhaTro = HDTT.maNhaTro where HDTT.sosinhvien >= 0 and NT.maChuNha = CN.maChuNha and CN.sdt like N'%' + dbo.fuConvertToUnsign1 (" + searchItem + ") + '%' or HDTT.sosinhvien is null and NT.maChuNha = CN.maChuNha and CN.sdt like N'%' + dbo.fuConvertToUnsign1 (" + searchItem + ") + '%'";
            DataTable data;
